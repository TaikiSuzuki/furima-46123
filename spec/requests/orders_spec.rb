...
RSpec.describe "Orders", type: :request do
  ...
  describe "GET /items/:item_id/orders" do
    context 'ログイン済みのユーザーの場合' do
      ...
      it '自身が出品していない販売中の商品の購入ページに遷移できること' do
        get item_orders_path(item)
        expect(response).to have_http_status(200)
      end

      it '自身が出品した商品の購入ページに遷移しようとするとトップページにリダイレクトされること' do
        my_item = create(:item, user: user)
        get item_orders_path(my_item)
        expect(response).to redirect_to(root_path)
      end

      it '売却済み商品の購入ページに遷移しようとするとトップページにリダイレクトされること' do
  ...
  describe "POST /items/:item_id/orders" do
    ...
    context '有効なパラメータの場合' do
      ...
      it 'データベースにOrderとShippingAddressのレコードが保存されること' do
        expect {
